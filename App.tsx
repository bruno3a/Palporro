...
import { PILOTS, INITIAL_TRACKS, SCRIPT_SYSTEM_INSTRUCTION_DYNAMIC, VOICE_OPTIONS, ANALYSIS_SYSTEM_INSTRUCTION } from './constants';
import { decode, encode, decodeAudioData, createWavBlob } from './audioUtils';
import { TrackStatus, Standing } from './types';
import { getVotes, addVote, subscribeToVotes, getVotingStats, getEnvironment, getVoteByIp } from "./src/supabaseClient";
...
const arraysEqual = (a: string[], b: string[]) => {
  ...
  useEffect(() => {
    ...
    const hasSelection = votingState.selectedDays.length > 0 || votingState.selectedTimes.length > 0;
    if (!existing) {
      setPendingVoteChange(hasSelection);
      return;
    }
    const sameDays = arraysEqual(existing.days, votingState.selectedDays);
    const sameTimes = arraysEqual(existing.times, votingState.selectedTimes);
    setPendingVoteChange(hasSelection && !(sameDays && sameTimes));
  }, [votingState.selectedDays, votingState.selectedTimes, votingState.userPilot, votingState.allVotes]);

  const computePendingChange = (): boolean => {
    const pilotName = votingState.userPilot;
    if (!pilotName) return false;
    const existing = votingState.allVotes.find(v => v.pilot === pilotName);
    if (!existing) return false;
    const sameDays = arraysEqual(existing.days, votingState.selectedDays);
    const sameTimes = arraysEqual(existing.times, votingState.selectedTimes);
    const atLeastOneChange = votingState.selectedDays.length > 0 || votingState.selectedTimes.length > 0;
    return atLeastOneChange && !(sameDays && sameTimes);
  };
  ...
  const isVotingActive = (): boolean => {
    ...
    // TEMPORAL: Habilitado toda esta semana (eliminar después)
    return true;
    
    // Domingo = 0, activo desde las 14hs
    // return dayOfWeek === 0 && hour >= 14;
  };
  ...
  const handleDeleteEmission = (id: string) => {
    setEmissionHistory(prev => prev.filter(e => e.id !== id));
  };
  ...
  const handleVoteSubmit = async () => {
    console.log('handleVoteSubmit triggered', {
      selectedDays: votingState.selectedDays,
      selectedTimes: votingState.selectedTimes,
      userPilot: votingState.userPilot,
      isSubmittingVote
    });
    if (votingState.selectedDays.length === 0 || votingState.selectedTimes.length === 0) {
    ...
    const voteData: VoteData = {
      days: votingState.selectedDays,
      times: votingState.selectedTimes,
  ...
  const getNextRaceInfo = () => {
    ...
    for (const day of VOTING_DAYS) {
      for (const time of VOTING_TIMES) {
        const count = votingState.allVotes.filter(v => v.days.includes(day) && v.times.includes(time)).length;
  ...
  const getPilotConfirmation = (pilotName: string) => {
    ...
    const hasDay = vote.days.includes(nextRace.day);
    const hasTime = vote.times.includes(nextRace.time);
  ...
  useEffect(() => {
    const checkAndClear = () => {
      ...
      if (!next) return;
      // If the scheduled race date/time is in the past, clear all votes to reset tentative/confirmed tags
      if (next.date.getTime() <= Date.now()) {
        setVotingState(prev => ({ ...prev, allVotes: [], hasVoted: false }));
      ...
      if (userIp) {
        ...
        if (existingVote) {
          ...
          setVotingState(prev => ({
            ...
            selectedDays: existingVote.days,
            selectedTimes: existingVote.times
          }));
          setShowPilotSelector(false);
          const stats = await getVotingStats(environment);
  ...
  const hasVoteChanged = (): boolean => {
    ...
    const daysChanged = JSON.stringify([...votingState.selectedDays].sort()) !== 
                        JSON.stringify([...currentVote.days].sort());
    const timesChanged = JSON.stringify([...votingState.selectedTimes].sort()) !== 
                         JSON.stringify([...currentVote.times].sort());
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-red-600 overflow-x-hidden" style={{ cursor: "url('/GTR34.webp'), auto" }}>
            ...
            <div className={`bg-zinc-900 border-2 border-red-600/50 rounded-[2rem] p-6 shadow-[0_0_60px_rgba(220,38,38,0.4)] backdrop-blur-xl bg-opacity-95 w-[400px]`}>
              <div className="flex items-start justify-between mb-4 cursor-grab active:cursor-grabbing" onMouseDown={handleMouseDown}>
                <div className="flex-1 pointer-events-none">
                  ...
                  <p className="text-[11px] text-zinc-500 font-mono">{displayedHistory[0].voice}</p>
                </div>
      ...
      {isVotingActive() && (
        <>
          ...
          {/* Panel de votación */}
          <div
            className={`fixed right-0 top-0 bottom-0 w-full md:w-[450px] bg-zinc-900 border-l-2 border-red-600/50 shadow-[0_0_60px_rgba(220,38,38,0.4)] z-50 transition-transform duration-500 overflow-y-auto ${
            ...
            <section className={`w-full bg-zinc-900 border ${fiaScoreData.isPending ? 'border-zinc-800' : 'border-red-600/30'} rounded-[2.5rem] p-6 md:p-14 shadow-[0_0_60px_rgba(0,0,0,0.6)] relative overflow-hidden transition-all duration-700 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8`}>
               <div className="relative z-10 text-center md:text-left flex-1">
                  ...
                  <h3 className="text-5xl md:text-8xl font-black italic uppercase text-white tracking-tighter leading-none">
                      Fia<span className="text-red-600 drop-shadow-[0_0_20px_rgba(220,38,38,0.6)]">Score</span>
                  </h3>
                  <div className="mt-6 md:mt-8 bg-black/30 p-4 md:p-6 rounded-2xl md:rounded-3xl border-l-4 border-red-600 inline-block backdrop-blur-md hidden md:block">
                    <p className="text-[12px] md:text-[16px] text-zinc-300 font-black uppercase tracking-[0.1em] max-w-xl italic leading-relaxed">
                      "Sistema neuronal de análisis predictivo: Evaluando la adhesión de pilotos, el respeto en pista y el inminente riesgo de fracaso institucional."
                    </p>
                  </div>
               </div>
               ...
               <div className="relative z-10 flex flex-col items-center md:items-end w-full md:w-auto">
                  <div className="flex items-center gap-4 md:gap-6 mb-4 md:mb-6">
                    <span className={`text-6xl md:text-8xl font-black italic tabular-nums leading-none ${fiaScoreData.isPending ? 'text-zinc-800' : 'text-red-600'}`}>
                      {fiaScoreData.label}
                    </span>
                  </div>
                        ...
                        <tr key={s.pilot} className="group hover:bg-zinc-800/30 transition-all cursor-crosshair">
                          ...
                          <td className="px-3 md:px-6 py-4 md:py-6"><div className="text-[13px] md:text-[16px] font-black uppercase italic group-hover:text-red-500 transition-colors flex items-center gap-2 md:gap-3">{s.pilot}{idx === 0 && <span className="text-[7px] md:text-[8px] bg-red-600 text-white px-1.5 md:px-2 py-0.5 rounded italic font-black">P1</span>}
                            {(() => {
                              ...
                              const isConfirmed = nextRace && nextRace.isDefinitive && pilotVote && pilotVote.days.includes(nextRace.day) && pilotVote.times.includes(nextRace.time);
                ...
                return (
                  <div className="bg-zinc-950 p-6 md:p-8 rounded-2xl border border-zinc-800 flex flex-col md:flex-row items-start gap-6">
                    <div className="flex-1">
                      ...
                      <div className="text-zinc-300 italic mt-3 text-sm">"{first.script}"</div>
                ...
                <div className="space-y-4">
                  {emissionHistory.slice(1).map(e => (
                    <div key={e.id} className="bg-zinc-950 p-4 rounded-2xl border border-zinc-800 flex items-start gap-4">
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <div className="font-black uppercase">{e.voice}</div>
                          <div className="text-[10px] text-zinc-500">{new Date(e.timestamp).toLocaleString('es-AR')}</div>
                        </div>
                        <div className="text-zinc-300 italic mt-2">"{e.script}"</div>
